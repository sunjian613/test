...
ECHO.[ Exec ] adb push monkey.txt /data/local/abc/
adb push monkey.txt /data/local/

ECHO.[ INFO ] 启动另外的文件执行shell命令
ECHO.[ Exec ] adb shell < shell_monkey.txt
adb shell monkey -f /data/local/monkey.txt -v 2
